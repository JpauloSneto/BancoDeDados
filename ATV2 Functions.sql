...
-- 1. Function: CalcularIdadeCliente
-- Calcula a idade do cliente com base na data de cadastro (apenas para exemplo, idealmente seria data de nascimento).
DELIMITER //
...
-- 2. Function: CalcularValorTotalVendaProduto
-- Calcula o valor total de um item específico em uma venda (quantidade * valor unitário).
DELIMITER //
...
-- 3. Function: ObterEstoqueAtualProduto
...
-- 4. Function: ObterDescontoPromocaoAtiva
-- Retorna o maior desconto de promoção ativa para uma data específica.
DELIMITER //
...
-- 5. Function: FormatarTelefone
-- Formata um número de telefone para um padrão específico (ex: (XX) XXXXX-XXXX).
DELIMITER //
...
-- 6. Function: ObterPrecoComDescontoPromocional
-- Calcula o preço final de um produto considerando um desconto de promoção.
DELIMITER //
